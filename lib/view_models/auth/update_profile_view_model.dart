...
final updateProfileViewModelProvider =
    StateNotifierProvider.autoDispose<UpdateProfileViewModel, AsyncValue<String?>>(
...
class UpdateProfileViewModel extends StateNotifier<AsyncValue<String?>> {
  ...
  UpdateProfileViewModel({required this.ref}) : super(const AsyncValue.data(null));

  Future<void> updateProfile({required Map<String, dynamic> payload}) async {
    state = const AsyncValue.loading();
    try {
      final result = await ref.read(authRepoProvider).updateProfile(payload: payload);
      result.fold(
